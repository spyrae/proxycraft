...
class WhatsAppSubscription(Base):
    ...
    async def get_by_id(cls, session: AsyncSession, subscription_id: int) -> Self | None:
        query = await session.execute(
            select(WhatsAppSubscription).where(WhatsAppSubscription.id == subscription_id)
        )
    ...
    async def list_by_user(cls, session: AsyncSession, user_tg_id: int) -> list[Self]:
        query = await session.execute(
            select(WhatsAppSubscription)
            .where(WhatsAppSubscription.user_tg_id == user_tg_id)
            .order_by(desc(WhatsAppSubscription.activated_at), desc(WhatsAppSubscription.id))
        )
    ...
    async def update_expiry(
        ...
        await session.execute(
            ...
            .where(WhatsAppSubscription.id == sub.id)
    ...
    async def get_next_available_port(
        ...
        result = await session.execute(
            select(WhatsAppSubscription.port).where(WhatsAppSubscription.is_active == True)  # noqa: E712
        )
    ...
    async def has_trial_used(cls, session: AsyncSession, user_tg_id: int) -> bool:
        query = await session.execute(
            select(func.count(WhatsAppSubscription.id)).where(
                WhatsAppSubscription.user_tg_id == user_tg_id,
                WhatsAppSubscription.is_trial_used == True,  # noqa: E712
            )
        )
